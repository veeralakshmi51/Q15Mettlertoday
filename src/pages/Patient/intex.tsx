...
import 
{
  Table,Modal,ModalFooter, Row, Col, Card, CardBody, CardTitle, CardSubtitle, CardFooter, Badge, ModalHeader, ModalBody
} from "reactstrap";
...
const Patient: React.FC = () => {
  ...
  const handleSave = async() => {
    try {
      ...
      console.log("requestBody55",requestBody,userData)
...
const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleDischarge = async (room: string) => {
    const confirmDelete = window.confirm(
      "Are You Sure Do You Want to Discharge?"
    );
    if (confirmDelete) {
      try {
        await patientDischarge(dispatch,room,organization);
...
const handleAddress = (e:any) => {
  console.log(e.target); // Log event target to check if it's received correctly
              ...
              <tr key={index}>
                ...
                <Tooltip title="Edit" arrow>
                  ...
                  <FontAwesomeIcon
                      ...
                      className="text-danger"
                      style={{ cursor: "pointer" }}
                      onClick={() => handleDischarge(patient.assignedBed)}
                    />
                  </Tooltip>
                </td>
              </tr>
